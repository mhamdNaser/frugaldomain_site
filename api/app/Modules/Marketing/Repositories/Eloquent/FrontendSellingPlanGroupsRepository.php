...
class FrontendSellingPlanGroupsRepository implements SellingPlanGroupsRepositoryInterface
{
    public function __construct(
        protected SellingPlanGroup $model
    ) {}
        ...
        return $this->applyTenantScope($this->model->newQuery())
            ->when($search, function ($query) use ($search) {
                $query->where(function ($q) use ($search) {
                    $q->where('shopify_selling_plan_group_id', 'like', "%{$search}%")
